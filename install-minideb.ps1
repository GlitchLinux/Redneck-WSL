...
function Install-MasterComponents {
    try {
        ...
        # Download master.zip using PowerShell (reliable for single file)
        if (-not (Test-Path $Config.MasterZipPath)) {
            Show-Progress -Activity "Downloading Components" -Status "Downloading master.zip..." -PercentComplete 0
            
            Invoke-WebRequest -Uri $Config.MasterUrl -OutFile $Config.MasterZipPath -UseBasicParsing -TimeoutSec 600
            
            if (Test-Path $Config.MasterZipPath) {
                $fileSize = [math]::Round((Get-Item $Config.MasterZipPath).Length / 1MB, 2)
                Write-Log "Master.zip downloaded successfully: $($Config.MasterZipPath) ($fileSize MB)" "SUCCESS"
            } else {
                Write-Log "Failed to download master.zip" "ERROR"
                return $false
            }
        } else {
        ...
        # Extract master.zip
        Show-Progress -Activity "Downloading Components" -Status "Extracting master.zip..." -PercentComplete 50
        ...
        Expand-Archive -Path $Config.MasterZipPath -DestinationPath $Config.InstallPath -Force
        ...
        Write-Log "Master.zip extracted successfully" "SUCCESS"
        
        # Copy components to their target locations
        Show-Progress -Activity "Downloading Components" -Status "Installing components..." -PercentComplete 75
        
        # Copy files from master/ to their proper locations
        $sourceFiles = @{
        ...
        # Setup wget in standard location
        if (-not (Test-Path $Config.WgetDir)) {
            New-Item -ItemType Directory -Path $Config.WgetDir -Force | Out-Null
        }
        
        $wgetSource = Join-Path $Config.MasterExtractPath "wget.exe"
        if (Test-Path $wgetSource) {
            Copy-Item $wgetSource $Config.WgetPath -Force
            Write-Log "Wget installed to $($Config.WgetPath)" "SUCCESS"
        }
        ...
        Show-Progress -Activity "Downloading Components" -Status "Complete" -PercentComplete 100
        ...
        if (-not $Quiet) {
            Write-Progress -Activity "Downloading Components" -Completed
        }
    }
}
